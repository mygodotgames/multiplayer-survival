...
func _ready():
	if is_multiplayer_authority():
		...
		# Eğer yoksa yenisini oluştur (Opsiyonel)
		if not inventory:
...
func on_item_picked_up(item: Item):
	if is_multiplayer_authority():
		if InventoryManager.player_inventory.add_item(item):
			print("Item envantere eklendi:", item.name)
		else:
			print("Envanter dolu! Item alınamadı:", item.name)
